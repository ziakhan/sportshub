...
const V = "?v=2" // bump to bust the optimizer cache when shots are recaptured

export function BrowserShot({ src, alt, caption }: { src: string; alt: string; caption: string }) {
  return (
    <figure className="min-w-0">
      <a href={src + V} target="_blank" rel="noreferrer" title="Open full size" className="border-ink-200 block cursor-zoom-in overflow-hidden rounded-xl border bg-white shadow-xl">
        ...
        <Image src={src + V} alt={alt} width={1440} height={900} className="h-auto w-full" />
      </a>
...
export function PhoneShot({ src, alt, caption }: { src: string; alt: string; caption: string }) {
  return (
    <figure className="mx-auto w-full max-w-[300px] min-w-0">
      <a href={src + V} target="_blank" rel="noreferrer" title="Open full size" className="block cursor-zoom-in rounded-[30px] border-[6px] border-[#101322] bg-[#101322] shadow-xl">
        <div className="overflow-hidden rounded-[24px]">
          <Image src={src + V} alt={alt} width={780} height={1688} className="h-auto w-full" />
        </div>
      </a>

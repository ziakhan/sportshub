...
async function authorizeStaff(tenantId: string, pollId: string, userId: string, isPlatformAdmin: boolean) {
  if (!(await canManageClubPolls(userId, tenantId, isPlatformAdmin))) {
...
/** PATCH /api/clubs/[id]/polls/[pollId] — staff close/reopen voting */
export async function PATCH(
  ...
  try {
    ...
    console.error("Club poll update error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}

/** DELETE /api/clubs/[id]/polls/[pollId] — staff; cascades questions/votes */
export async function DELETE(
  ...
  try {
    ...
    console.error("Club poll delete error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}

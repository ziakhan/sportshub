...
export async function GET(request: NextRequest) {
  ...
  if (!payload) {
    return NextResponse.redirect(new URL("/unsubscribed?status=invalid", publicOrigin(request)))
  }
  try {
    await withdrawConsent(payload.userId, payload.scope, payload.orgId, "unsubscribe-link")
  } catch (error) {
    console.error("Unsubscribe error:", error)
    return NextResponse.redirect(new URL("/unsubscribed?status=error", publicOrigin(request)))
  }
  return NextResponse.redirect(new URL("/unsubscribed?status=ok", publicOrigin(request)))
}

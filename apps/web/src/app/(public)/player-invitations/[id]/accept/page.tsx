...
export default async function AcceptPlayerInvitationPage({ params }: { params: { id: string } }) {
  ...
  const callbackUrl = `/player-invitations/${invitation.id}/accept`
  const inviterName =
    [invitation.invitedBy?.firstName, invitation.invitedBy?.lastName].filter(Boolean).join(" ") ||
    "A club admin"
  const feeLabel =
    invitation.seasonFee != null
      ? formatCurrency(Number(invitation.seasonFee), invitation.tenant.currency || "CAD")
      : null
  ...
  // Already responded (or expired) — summary, no actions
      ...
      <div className="container mx-auto max-w-xl px-4 py-12">
        <div className={cardClass}>
          ...
          <p className="text-ink-600 text-sm">
            This invitation to join {invitation.team.name} is no longer open. If you think this is a
      ...
      <div className="container mx-auto max-w-xl px-4 py-12">
        <div className={cardClass}>
          ...
          <p className="text-ink-600 mb-4 text-sm">
            This invitation was sent to <strong>{invitation.invitedEmail}</strong>. You are signed in
            as a different user. Sign out and sign in with that email to accept.
          </p>
          <Link
  ...
  // Signed in as the right user (or a fresh post-signup user whose invitedUserId
  // was attached by the signup auto-attach). Load their players for the picker.

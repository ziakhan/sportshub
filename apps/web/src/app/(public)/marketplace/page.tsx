...
export default async function PublicMarketplacePage({
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-ink-950">Tryout Marketplace</h1>
      ...
      {!singleCountry && enabledCountries.length > 1 && (
        <div className="mb-6 flex flex-wrap gap-1.5">
          <Link
            href="/marketplace"
            className={`rounded-full px-3 py-1 text-xs font-medium ${!searchParams.country ? "bg-play-600 text-white" : "bg-ink-100 text-ink-700 hover:bg-ink-200"}`}
          >
            All
          </Link>
          {enabledCountries.map((c) => (
            <Link
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium ${searchParams.country === c.code ? "bg-play-600 text-white" : "bg-ink-100 text-ink-700 hover:bg-ink-200"}`}
            >
              {c.code}
            </Link>
          ))}
      ...
      {tryouts.length === 0 ? (
        <div className="rounded-[28px] border-2 border-dashed border-ink-200 bg-white p-12 text-center">
          <h3 className="text-lg font-bold text-ink-950 mb-2">No tryouts available</h3>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {tryouts.map((tryout: any) => (
            ...
            >
              ...
              <div className="p-6">
                <h4 className="text-xl font-bold text-ink-950 mb-2">{tryout.title}</h4>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-ink-100">
                  <div className="text-2xl font-bold text-hoop-600">

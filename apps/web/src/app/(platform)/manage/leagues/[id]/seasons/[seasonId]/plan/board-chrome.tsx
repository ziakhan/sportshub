...
import { WhyPopover, ActionPopover, QuietAction } from "./plan-ui"
    ...
    <div className="border-ink-200 bg-ink-50/60 flex flex-wrap items-center justify-between gap-3 border-b px-5 py-4">
      ...
      <div className="flex flex-col items-end gap-1">
        <div className="flex flex-wrap items-center justify-end gap-2.5">
          ...
          {/* THE REDRAW MENU (approved design 2026-08-10, toolbar diet):
              redraw, its one alternative shape, and fill-from-pool live
              behind one quiet trigger — the generate CTA stays the page's
              only loud verb. Same testids, same behaviors, one button. */}
          {planId && interactive && (worldUsable || view === "board") && (
            <ActionPopover
                ...
                <div className="space-y-1.5">
                  {worldUsable && (
                    <button
                      ...
                      className="border-ink-200 hover:border-play-400 block w-full rounded-lg border bg-white px-2.5 py-2 text-left text-[12.5px] font-bold"
                    >
                      {COPY.redraw}
                      <span className="text-ink-500 block text-[11px] font-normal">{COPY.drawHint}</span>
                    </button>
                  )}
                  {worldUsable && (
                    <button
                  ...
                  {view === "board" && (
                    ...
                    >
                      {COPY.fillFromPool}
                      <span className="text-ink-500 block text-[11px] font-normal">{COPY.fillHint}</span>
                    </button>
                  )}
                </div>
              )}

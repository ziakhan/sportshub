...
export default function TournamentManagePage() {
  ...
  if (!tournament)
    return <div className="text-ink-500 py-12 text-center p-6">Tournament not found.</div>
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-ink-900">{tournament.name}</h1>
          <p className="text-sm text-ink-500">
            {tournament.city}
            {tournament.state ? `, ${tournament.state}` : ""}
          </p>
          <span className="mt-1 inline-block rounded-full bg-play-100 px-3 py-0.5 text-xs font-medium text-play-700">
      ...
      <div className="grid grid-cols-1 gap-4 mb-8 sm:grid-cols-3">
        <div className="rounded-lg border bg-white p-4 text-center">
          <div className="text-2xl font-bold text-play-700">{divisions.length}</div>
          <div className="text-xs text-ink-500">Divisions</div>
        </div>
        <div className="rounded-lg border bg-white p-4 text-center">
        ...
        <div className="rounded-lg border bg-white p-4 text-center">
          ...
          <div className="text-xs text-ink-500">Venues</div>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-ink-200 bg-white p-6">
          <h3 className="font-semibold text-ink-900 mb-4">Divisions</h3>
          {divisions.length === 0 && (
            <p className="text-sm text-ink-500 mb-4">No divisions added yet.</p>
          )}
          {divisions.map((d: any) => (
            <div
              key={d.id}
              className="flex items-center justify-between rounded-md bg-court-50 px-3 py-2 mb-2"
            >
              <div>
              ...
              <div className="flex items-center gap-2">
                {d.maxTeams && (
                  <span className="text-xs text-ink-400">max {d.maxTeams}</span>
                )}
                ...
                <button
                  ...
                  className="text-xs text-red-500 hover:text-hoop-700"
                >
                  Remove
                </button>
              </div>
          ...
          <div className="mt-4 space-y-2 border-t pt-4">
            ...
            <button
              onClick={addDivision}
              className="w-full rounded-xl bg-play-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-play-700"
            >
              Add Division
            </button>
          </div>
        ...
        {/* Registered Teams */}
        <div className="rounded-lg border border-ink-200 bg-white p-6">
          <h3 className="font-semibold text-ink-900 mb-4">Registered Teams</h3>
          {teams.length === 0 ? (
            <p className="text-sm text-ink-500">No teams registered yet.</p>
          ) : (
            teams.map((t: any) => (
              <div
                key={t.id}
              ...
              >
                <div>
                  <span className="font-medium text-ink-900">{t.team?.name || "Unknown"}</span>
                  <span className="ml-2 text-xs text-ink-500">
                    {t.team?.tenant?.name}
                  </span>
                  {t.division && (
                    <span className="ml-2 text-xs text-play-700">{t.division.name}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <span
                  ...
                  {t.status === "PENDING" && (
                    <>
                      <button
                        ...
                        className="rounded px-2 py-0.5 text-xs font-medium bg-court-100 text-court-700 hover:bg-green-200"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => updateTeamStatus(t.id, "REJECTED")}
                        className="rounded px-2 py-0.5 text-xs font-medium bg-hoop-100 text-hoop-700 hover:bg-red-200"
                      >
                        Reject
                      </button>
                    </>
        ...
        {/* Venues */}
        <div className="rounded-lg border border-ink-200 bg-white p-6 lg:col-span-2">
          <h3 className="font-semibold text-ink-900 mb-4">Venues</h3>
          {venues.length === 0 && (
            <p className="text-sm text-ink-500 mb-4">No venues added yet.</p>
          )}
      ...
      <div className="mt-6 rounded-lg border border-ink-200 bg-white p-6">
        <h3 className="font-semibold text-ink-900 mb-3">Tournament Summary</h3>
        <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
          ...
          {tournament.gameLengthMinutes && (
            <div>
              <span className="text-ink-500">Game Format:</span>{" "}

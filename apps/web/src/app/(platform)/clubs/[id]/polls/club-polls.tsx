...
/**
 * Club-wide polls (three-tier polls ruling, owner 2026-07-24). Voting/
 * display reuses the shared PollCard; creation adds the club-only "Also
 * post to team chats" relay (default OFF).
 */
...
export function ClubPolls({ clubId }: { clubId: string }) {
  ...
  async function submitVote(poll: PollView) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/clubs/${clubId}/polls/${poll.id}/vote`, {
        method: "POST",
  ...
  async function managePoll(poll: PollView, action: "close" | "reopen" | "delete") {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/clubs/${clubId}/polls/${poll.id}`, {
    ...
    <div className="space-y-4">
      ...
      {isStaff && showCreate && (
        <CreatePollForm
          clubId={clubId}
          teams={teams}
          onCreated={(poll, relayedTeams) => {
            setShowCreate(false)
            setPolls((current) => [poll, ...current])
            setSelections((current) => ({ ...current, [poll.id]: seedSelections(poll) }))
            setNotice(
              relayedTeams > 0
                ? `Poll published. Also posted to ${relayedTeams} team chat${relayedTeams === 1 ? "" : "s"}.`
                : "Poll published."
            )
          }}
        />
      ...
      {polls.length === 0 && !showCreate && (
        <div className="border-ink-200 rounded-2xl border border-dashed bg-white px-6 py-12 text-center">
          <p className="text-ink-700 text-sm font-semibold">No club polls yet</p>
          <p className="text-ink-500 mt-1 text-sm">
            {isStaff
              ? "Ask the whole club something: season dates, uniform colors, event interest."
              : "When your club posts a poll, it'll show up here."}
          </p>
        </div>
      )}
...
function CreatePollForm({
  ...
  clubId: string
  teams: TeamOption[]
  onCreated: (poll: PollView, relayedTeams: number) => void
}) {
  const [title, setTitle] = useState("")
  const [description, setDescription] = useState("")
  const [questions, setQuestions] = useState<DraftQuestion[]>([emptyQuestion()])
  const [relay, setRelay] = useState(false) // default OFF (owner ruling)
  const [relayTeamIds, setRelayTeamIds] = useState<string[]>([])
  ...
  function toggleRelayTeam(teamId: string, checked: boolean) {
    setRelayTeamIds((current) =>
    ...
    try {
      const res = await fetch(`/api/clubs/${clubId}/polls`, {
        ...
        body: JSON.stringify({
          ...
          relayTeamIds: relay ? relayTeamIds : [],
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      onCreated(data.poll, data.relayedTeams ?? 0)
    } catch (e: any) {
    ...
    <div className="border-ink-100 space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
      <div>
        ...
        <input
          ...
          maxLength={150}
          placeholder="Which weekend works for the season kickoff?"
      ...
      {questions.map((question, qi) => (
        <div key={qi} className="border-ink-100 bg-ink-50/50 space-y-2 rounded-xl border p-3">
          ...
          <input
            ...
            maxLength={300}
            placeholder="What should we ask the club?"
            className={inputClass}
          />
      ...
      <div className="border-ink-100 rounded-xl border p-3">
        <label className="text-ink-700 flex items-center gap-2 text-sm font-semibold">
        ...
        {relay && (
          <div className="mt-2 max-h-40 space-y-1 overflow-y-auto">
            {teams.length === 0 ? (
              <p className="text-ink-400 text-xs">No teams yet.</p>
            ) : (
              teams.map((t) => (
                <label key={t.id} className="text-ink-600 flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={relayTeamIds.includes(t.id)}
                    onChange={(e) => toggleRelayTeam(t.id, e.target.checked)}
                  />
                  {t.name}
                </label>
              ))

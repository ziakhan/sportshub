...
export const DEMOS: DemoEntry[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Every playoff game has its day and time before anybody knows who is in it",
      "The whole weekend checked against gym time that is actually booked",
    ],
    chapterTitles: ["The last weekend", "Everybody plays"],
    thumbEyebrow: "Chapter 9",
  },
  {
    slug: "league-rulebook",
    title: "The rules a league writes down",
    promise:
      "Two teams finish level and nothing says who is first. The league writes the ladder once, the table re-reads itself with the deciding rule named on every row, and a player who missed half the season is ruled on in writing.",
    description:
      "These are the decisions a league makes once and then lives with, shown at the moment they finally matter. Two teams finish seven and three and the Tiebreakers column is empty, so the order between them is arbitrary. The rules live in one screen: head to head first, then the ladder under it, applied top to bottom until one team wins, ending in a rule that always answers. Six of them, and each can be moved or removed until the season is finalized. The table then re-reads itself and names the rule that placed each row, including a cluster of four teams all finished six and four. The second half is who is allowed on the floor. Games played come from the scorekeeper's roll call rather than a typed list, so a player short of the minimum is short for a reason the record can show. The badge is the control and the league can overrule it either way, but the ruling will not save without a written reason, and the star that marks it stays on her record.",
    audiences: ["leagues"],
    kind: "chapter",
    stage: "desktop",
    durationLabel: "51 sec",
    status: "live",
    chapterCount: 2,
    bullets: [
      "A tie with no rules written down is arbitrary, and the product says so",
      "The ladder written once, in order, ending in a rule that always answers",
      "The table re-reads itself and names the rule that placed each row",
      "Playoff eligibility from the scorekeeper's roll call, not a typed list",
      "A ruling that will not save without a written reason, kept on the record",
    ],
    chapterTitles: ["The rule that decides", "Who can play"],
  ...
  },
  {
    ...
    bullets: [
      "The document built from Ontario's own template, versioned, renewing every year under Rowan's Law",

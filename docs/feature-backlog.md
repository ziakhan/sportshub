...
## Team schedule requests — approved, budgeted, best-effort (owner 2026-08-01 — NOT scheduled)
Travel teams (e.g., from Ottawa) need things like an early Sunday game (leave early) or a later Saturday start. Cannot be open to all teams or everyone requests the good slots. Shape: team submits a request → league approves or denies → only approved requests feed the scheduler, advertised as best effort with no guarantees. The gating question before the feature ships: how many requests can be approved without penalizing other teams — approvals need a feasibility budget, not just a yes button. Likely mechanism: simulate the schedule with the candidate request and show the league its cost to everyone else before approving (engine runs ~1-2.5s, interactive); fairness report gains "requests honored n/m." Builds on the parked per-team blackout/time-window item from Schedule Quality v2 and the day-anchor pre-plan.

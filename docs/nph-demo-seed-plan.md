...
> — world builds in ~30s, reset-idempotent. Local noise scrubbed (1,668 test
...
>
> **v3 (owner demo-readiness round, 2026-07-07):** leagues renamed — active =
> **NPH Summer League** (Summer 2026, ends late July, 5 weekly weekend
> sessions, 10 games/team = 2/weekend), open = **NPH Fall League** (Fall 2026,
> Oct–Mar, 6 monthly sessions, 12 games/team target). Games now generated by
> the REAL scheduler over real substrate (SeasonVenue + courts + sessions →
> Venues/Sessions/Schedule tabs all populated; Find Alternates works). League
> fees carry Payment rows → league payments page shows $127,680 collected.
> Shipped with it: viewer-aware box-score names (parents see full names; C.K.
> double-initial bug fixed), tighter leaders UI, My Hub parent door, forfeit
> relabels + confirms, locked-season guards on Divisions/Sessions tabs,
> floating chat dock on public pages, scheduler pairing-balance fix
> (circle-method rounds). Playoffs = next feature (owner-deferred).

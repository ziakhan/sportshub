...
## 29. 2026-08-01 — TEAM SCHEDULE REQUESTS + SCENARIO RECOMMENDATIONS + ORG CAPACITY PLANNER v1 (owner: "build this whole thing out")
Three pillars, one pass (plan declarative-gliding-wren v2):
- **Schedule requests (best effort, approval-gated)**: new `TeamScheduleRequest` (WINDOW/BLACKOUT, weekday XOR date, WithdrawalRequest-style workflow) + `TeamSubmission.scheduleRequestsEnabled` (OFF by default; league flips per team — the club-side UI is invisible until then) + `SeasonTeamBlackout.sourceRequestId`. Club side: team-dashboard button → preset-only request page (Early Sunday / Later Saturday / Blackout a date — owner: "very limited, they cannot really choose"), best-effort copy everywhere, cancel-while-pending. League side: enable checkbox, request panel with **Simulate cost** (engine runs with/without the pending request, same seed; diff of unplaced/b2b/preference/request misses shown before Approve), blackout manager (league-direct add/remove; existing SeasonTeamBlackout model was stored-but-never-scheduled — now wired). Approving a BLACKOUT materializes blackout rows (recurring → one per matching session day); withdrawal cascades cancel pending requests; bell + email on decisions.
- **Engine**: blackouts = HARD law (every pass except relaxed repair; anchor pre-plan avoids blacked days; tier-0 movers refuse). Windows = hard in BOTH placement passes (soft −12 lost to "only candidate for the slot" — receipts), repair-only concession with loud trade-off notes; **window-aware anchor day choice** (a "Saturdays after 2pm" team anchors to Sundays — receipts went 2 violations → 0) + request compliance ranks SECOND in the retry wrapper's lexicographic pick, right after unplaced games. Fairness report gains per-team "requests honored n/m" + `requestViolations` total.
- **Scenarios (owner: "not an option — a recommendation")**: POST schedule/scenarios runs baseline + compact-day + free-a-court/venue + trim-hours variants (each a full engine run; only quality-holding variants offered; wins like "Paramount stays completely free all season" / "nothing after 3:00 p.m."); descriptors round-trip through preview/commit so "Use this scenario" saves exactly what was shown. Schedule tab: Scenarios button + comparison cards + active-scenario chip. Engine overrides: `excludeCourtIds`, `dayWindow` clamp, `compactDays` nudge.
- **Org capacity planner v1** (/manage/org/[id]/planner + POST /api/organizations/[id]/planner/run): all the org's schedulable seasons in one pool, run SEQUENTIALLY with earlier seasons' games injected as hard court bookings (the existing cross-league busyCourtBookings channel), levers (hold courts free, day window, compact), per-season fit lines, court utilization table (used/capacity %), recommendations (idle/under-15% courts, doesn't-fit advice). Pure simulation, nothing persists.
- **Receipts**: engine 50/50 pins (blackout hard, window honored, impossible window reported, grid overrides, compact ≤ baseline); int 386/386 (13 new: gate/dupe/authz/materialization/cascade/simulate/scenarios); Playwright verify-schedule-requests.mjs **14/14** — club sees+submits, league simulates (diff shown), approves, regenerated schedule honors (Saturday 16:30 ≥ 14:00), scenarios return variants, planner allFit=true across Fall+Summer with 8 courts; screenshots /tmp/schedule-requests-verify. Demo world: Force G9 APPROVED Sunday-≤12:00 (Ottawa story), Force G10 PENDING Saturday-≥14:00 (live demo of simulate→approve), Polaris league-added blackout.

## 30. 2026-08-01 — BACK-TO-BACKS AT ALL COSTS (owner priority ruling) + four bug fixes
Owner rulings after testing on the box: back-to-backs are avoided AT ALL COSTS; split-days must never outrank them; approved requests are best effort ("no requests are 100%"), never honored at the price of OTHER teams' back-to-backs; requests apply only after approval. Investigation receipts that drove the fixes: mixed preferences (not split-days itself) created the tension — uniform worlds scheduled clean both ways; 3 of the 4 box back-to-backs had a FREE same-day slot nobody tried; the what-if with uniform long hours still kept 4 → pure search gap, not capacity.
- **Priority reorder**: retry criterion now [unplaced, back-to-backs, request misses, style misses, edge spread, notes]; b2b hard in BOTH placement passes; mover tiers [core+window, core-only, anything] so repair breaks a WINDOW before it ever creates a back-to-back; repair endgame bonus 2000/1000 core/window.
- **B2B ELIMINATION pass** (new, runs LAST among time-movers): finds every adjacent pair, retimes one game — same-day window-kept first, same-day, then the weekend's other day (breaking one-trip is allowed: b2b outranks every preference), cross-day window-broken last. Edge-rebalance now refuses swaps that would CREATE adjacency (it was un-fixing the fixer).
- **Hours propagation fix**: venue-hours edits (setup card + hours PUT) now push into every existing SeasonSessionDayVenue row (applyVenueHoursToSessionDays; responses carry updatedDays) — the owner's 8:00-20:00 edit had reached only half the weekends, so the engine ran mixed hours and scenario advice read as nonsense.
- **Loader determinism**: loadSchedulerInput orders divisions/submissions/sessions/days/dayVenues/courts/blackouts/requests explicitly — Postgres row order was changing schedules for the same seed (box ≠ local).
- **Trim-card guard**: the shrink-hours scenario is only offered when it actually saves court time vs the configured close.
- **Receipts (box's EXACT exported world, fixed engine)**: as-is mixed world 4 b2b → **0 b2b, request honored, 0 unplaced**; all 6 battery scenarios (uniform styles, no requests, uniform hours, split-everyone) = 0 b2b. Scheduler suite 51/51 (new pin: a window BREAKS rather than take a back-to-back, and says so).

## 31. 2026-08-01 — DEMO JOURNEY ARC (waves 1-4): scenario loader, full-scale real-NPH world, staged pitch script
Owner: "press a button in my admin console and load that demo… full-scale every league with the correct names." Player names FICTIONAL (ruled — minors); team/club/venue names REAL from the census.
- **Wave 1**: census codegen (scripts/data/nph-census.ts — 230 real entries, totals asserted vs census doc) · seed-nph-demo --scenario/--stage + scripts/seed-journey.ts (stage 1: 121 real-name tenants, D1/NPA/WNPA approved + sessioned, SL 25/146 mid-registration, 29s; stage 2 additive top-up to 146; stage 3: all approved, FINALIZED, Six Park attached 5-of-6 courts ON PURPOSE, Ottawa-region requests + pending withdrawal seeded) · admin loader /dashboard/admin/demos (stage rail + fast-forward; POST api/admin/demos/load spawns the seeder detached — first child_process in the app; status file + world marker PlatformSettings.demoState [new col]) · engine structural-shortfall fast path (short world fails in 4.6s with "add a court" arithmetic instead of grinding 10+ min; retry cut at 2 attempts when games strand).
- **Wave 2**: venue distribution — lib/scheduler/distribute.ts (best-fit by load ratio capped at capacity/1.15; per-venue-group engine runs merged — groups share no courts so never double-book) + "Distribute by venue" scenario card + scenario.divisionVenueMap on preview/commit. RECEIPT (144 real teams, 720 games, 3 venues): 720/720, 0 b2b, 0 two-gym, 0 request misses, 0 preference misses, venues 79-83%, 10.7s.
- **Wave 3**: mostly existed (personal ICS feed incl. referee lens; forfeit = DEFAULTED+defaultedBy with standings + notifications; cancel/postpone fanout). Added Game.statusReason (reason → bell + email) + POST/DELETE /api/games/[id]/officials (game-scoped Referee roles; flows to ref calendar/ICS automatically).

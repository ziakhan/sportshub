...
## #56 — 2026-08-01: SCHEDULE QUALITY v2 (per-team weekend preferences) — ⏳ NOT DEPLOYED (needs owner word + BOX SCHEMA PUSH + reseed)
...
- Deploy steps when approved: push GitHub → box `prisma db push` → deploy.sh → reseed demo world (`npx tsx scripts/seed-nph-demo.ts`).

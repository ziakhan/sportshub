...
### T-015 · Planner "weekends" can't express real session day-shapes (Fri–Sat, Sun–Mon, Fri–Sun) · IDEA · high · web-desktop
- **Where:** LOCAL, latest master — Plan Your Season, the weekend/day model behind steps 2–3.
- **What I hit:** Real sessions dictated by building availability don't always fall Sat–Sun. Concrete cases from my plan: **Oct 9–10 = Friday + Saturday (no Sunday)**; **Nov 8–9 = Sunday + Monday (no Saturday)**; and sessions that run **Friday through Sunday**.
- **What the model supports today (code-checked):** weekends carry a `dayCount`, and there's an "add a Friday" mechanism (`fridays` / `withFridayBlocksInWorld` — "a Friday is extra capacity at a gym the weekend already has"). So Fri–Sun works via the Friday add-on; Fri–Sat only by adding Friday and manually zeroing Sunday's hours/courts (faking absence); **Sun–Mon is impossible — Monday doesn't exist in the model.**
- **Suggestion:** generalize the Friday mechanism into per-weekend DAY TOGGLES — each weekend unit declares which adjacent days it actually runs (Fri / Sat / Sun / Mon), with capacity, the draw, burden scoring, and the ask sheet all reading the declared set. The Friday add-on already proves the pattern; this finishes it. Fits the platform's own research conclusion that building availability drives the calendar — the days a league runs are a fact about the building, not a preference.
- **Severity note:** high — a league whose building gives them Sunday+Monday simply cannot plan that session truthfully today.

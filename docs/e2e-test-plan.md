...
| --- | ------------------------------------------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------- |
| 5.1 | **Club reviews tryout signups** — list all signups for a tryout     | ✅ Built   | Signups management UI exists and supports making offers                                               |
| 5.2 | **Club makes offer to player** — seasonFee, installments, expiresAt | ✅ Built   | `POST /api/offers`                                                                                    |
| 5.3 | **Offer notification sent** — in-app + email                        | ⚠️ Partial | In-app notification exists, email send is wired but delivery/provider validation still needs coverage |
| 5.4 | **Parent/Player views offer**                                       | ✅ Built   | `/offers` page                                                                                        |
| 5.5 | **Parent/Player accepts offer**                                     | ✅ Built   | `PATCH /api/offers/[id]`                                                                              |
| 5.6 | **Parent/Player declines offer**                                    | ✅ Built   | `PATCH /api/offers/[id]`                                                                              |
| 5.7 | **Offer expires automatically**                                     | ⚠️ Partial | Expiry is enforced on response and team finalization, but no background expiry job                    |
| 5.8 | **Accepted offer → TeamPlayer created**                             | ✅ Built   | Accept flow creates or updates `TeamPlayer`                                                           |
| 5.9 | **Club views finalized roster**                                     | ✅ Built   | Team roster page exists; jersey finalization flow is also present                                     |
...
1. **Payment flows** (Phase 6) — wire Stripe checkout into tryout signup and offer acceptance
2. **League management APIs + UI** (Phase 7) — division CRUD, team submission, approval
3. **Email delivery hardening** (Phase 10) — pick/validate provider behavior and templates in real environments
4. **Scheduling algorithm** (Phase 8) — round-robin generator per division
5. **Game day APIs** (Phase 9) — scoring, stats, standings
6. **Offer flow test coverage** — add route/integration tests around create/respond/expiry behavior
